<template>
  <el-dialog id="merge-rule-dialog" v-loading="loading" :show-close="true" :visible="isOpenMergeRuleEditDialog"
    element-loading-background="rgba(0, 0, 0, 0.8)" element-loading-spinner="el-icon-loading" element-loading-text="正在添加中"
    title="编辑并集规则" @close="handleClose">

    <el-form ref="mergeRuleAddForm" :model="mergeRuleAddForm" class="merge-rule-form" label-position="left"
      label-width="150px">
      <div class="content">
                    ...
                    <div class="output">
                      ...
                      <div class="top_body">
                        ...
                        <div class="cond-statements">
                          ...
                          <div>
                            <el-button icon="el-icon-plus" size="mini" type="text" @click="addExtraInput">
                              添加一行
                            </el-button>
                            <el-popover placement="top-start" title="" trigger="hover" width="500">
                              <el-alert :closable="false" title="每行为一个已有规则，每条规则相并" type="info">
                                <template slot='title'>

                                </template>
                              </el-alert>
                              <i slot="reference" class="el-icon-info" style="margin-left: 5px"></i>
                            </el-popover>
                          </div>
                          <div
                            v-if="mergeRuleAddForm.alarmRuleDTO.rules && mergeRuleAddForm.alarmRuleDTO.rules.length > 0">
                            <el-alert :closable="false" :show-icon="false" title="" type="info">
                              <template slot='title'>

                                <el-form-item label="" prop="alarmRuleDTO.rules">
                                  <div v-for="(item, index) in mergeRuleAddForm.alarmRuleDTO.rules" :key="index"
                                    style="margin-top: 5px">
                                    <el-select v-model="mergeRuleAddForm.alarmRuleDTO.rules[index]" placeholder="已有规则">
                                      <el-option v-for="item in options" :key="item.id" :label="item.alarmName"
                                        :value="item.id">
                                      </el-option>
                                    </el-select>
                                    <el-button icon="el-icon-delete" size="mini" style="color: #F56C6C" type="text"
                      ...
                      <div class="top_body">
                        <div class="condition-type">
                          <el-form-item label="通知方式" prop="alarmRuleDTO.alarmReceivers">
                            <el-select v-model="mergeRuleAddForm.alarmRuleDTO.alarmReceivers" multiple placeholder="请选择">
                          ...
                          <!-- <div class="type-radio">
                            <el-form-item label="报警方式" prop="alarmRuleDTO.alertType">
                              <el-radio-group v-model="mergeRuleAddForm.alarmRuleDTO.alertType" @change="changeType">
                ...
                <el-card class="box-card" shadow="never">
                  <div class="output">
                    <div class="top_head">警戒时间范围
                      <el-button icon="el-icon-plus" size="mini" type="text" @click="addTimeInput">
                        添加一行时间
                      </el-button>
                    </div>

                    <div v-for="(item, index) in mergeRuleAddForm.alarmTime" :key="index" class="alarm-time_body"
                      style="margin-top: 5px">
                      <el-row>

                        <el-col :span="12">
                          <el-form-item :prop="'alarmTime.' + index + '.dateType'" label="时间类型">
                            <el-radio v-model="mergeRuleAddForm.alarmTime[index].dateType" label="0">周末</el-radio>
                            <el-radio v-model="mergeRuleAddForm.alarmTime[index].dateType" label="1">工作日</el-radio>
                            <el-radio v-model="mergeRuleAddForm.alarmTime[index].dateType" label="2">节假日</el-radio>
                            <el-radio v-model="mergeRuleAddForm.alarmTime[index].dateType" label="3">每天</el-radio>
                          </el-form-item>
                        </el-col>
                        <el-col :span="12">
                          <el-form-item :prop="'alarmTime.' + index + '.mergeAlarmDate'" label="报警日期">
                            <el-date-picker v-model="mergeRuleAddForm.alarmTime[index].mergeAlarmDate" arrow-control
                              end-placeholder="结束时间" placeholder="选择时间范围" range-separator="至" start-placeholder="开始时间"
                              type="datetimerange" unlink-panels />
                          </el-form-item>
                        </el-col>
                        <el-col :span="12">
                          <el-form-item :prop="'alarmTime.' + index + '.mergeAlarmTime'" label="报警时间">
                            <el-time-picker v-model="mergeRuleAddForm.alarmTime[index].mergeAlarmTime"
                              end-placeholder="结束时间" is-range placeholder="选择时间范围" range-separator="至"
                              start-placeholder="开始时间" />
                          </el-form-item>
                        </el-col>
                        <el-col :span="12">
                          <el-button icon="el-icon-delete" size="mini" style="color: #F56C6C" type="text"
                            @click="delTimeInput(index)">删除
                          </el-button>
                        </el-col>
                      </el-row>
    ...
    <div slot="footer" class="dialog-footer">
      <el-button @click="isOpenMergeRuleEditDialog = false">取 消</el-button>
...
import { andRulesName, allContactor } from '@/api/rule/RuleConfig'
import { updateRule } from '@/api/rule/RuleConfig'

export default {
  name: 'MergeRuleAddDialog',
  props: {
    isOpenMergeRuleEditDialog: {
      type: Boolean,
      default: false
    },
  ...
  watch: {
    monitorGroupConfigFormData: {
      handler: function (newVal, oldVal) {
        ...
        this.mergeRuleAddForm = newVal
        console.log("this.mergeRuleAddForm", this.mergeRuleAddForm);
      },
      deep: true
    }
  },
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          //     break
          // }
          // if (this.mergeRuleAddForm.alarmTime.mergeAlarmDate && this.mergeRuleAddForm.alarmTime.mergeAlarmTime) {
          //   this.mergeRuleAddForm.alarmTime.startTime = new Date(this.mergeRuleAddForm.alarmTime.mergeAlarmDate[0]).getTime();
          //   this.mergeRuleAddForm.alarmTime.endTime = new Date(this.mergeRuleAddForm.alarmTime.mergeAlarmDate[1]).getTime();

          //   this.mergeRuleAddForm.alarmTime.alarmSt = new Date(this.mergeRuleAddForm.alarmTime.mergeAlarmTime[0]).getTime();
          //   this.mergeRuleAddForm.alarmTime.alarmEt = new Date(this.mergeRuleAddForm.alarmTime.mergeAlarmTime[1]).getTime();

          // }
          console.log(this.mergeRuleAddForm);

          updateRule(this.mergeRuleAddForm).then((res) => {
            if (res.code === 200) {
              this.$message({
                duration: 3000,
                type: 'success',
  ...
  created() {
    ...
    this.mergeRuleAddForm = deepCopy(this.monitorGroupConfigFormData)
    console.log("this.monitorGroupConfigFormData");
    console.log(this.monitorGroupConfigFormData);
    // console.log("this.mergeRuleAddForm");
    // console.log(this.mergeRuleAddForm);
